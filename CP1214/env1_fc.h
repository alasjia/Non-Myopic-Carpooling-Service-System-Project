...
/*定义各类常量*/
#define NUMREQ 49062       // 49062 = 25000 + 24062  /*请求次数,18-19为3550*/
#define NUMREQ_RUN 49062   // 实际运行的请求数
#define NUMCAR 500         /*系统中的车辆数*/
#define NUMTASK 98124      // 98124 = 50000 + 48124 /*任务数量,一个请求包括上车、下车两个任务   18-19为7100*/
#define NUMTASK_RUN 98124  // 实际运行的任务数
#define NUMNODE 225        /*gird站点数量，列数*/
#define WAITTHRESHOLD 10   /*容忍等待时间,min*/
#define DETOURTHRESHOLD 10 /*容忍绕路时间,min*/
#define FEE 3              /*车辆每公里运营费用，单位为【元/km】，乘客上车之后的收费*/
...
int TaskChain[50];       // 存储可能的任务链，每个车辆初始化一次
...
void reqdata_input();
void mincost_input();
// void init_arr(int Output_C_Distribution[NUMNODE], int Output_R_Distribution[NUMNODE], int Output_C_Location[NUMNODE]);
void init_arr(float Output_C_Distribution[NUMNODE], float Output_R_Distribution[NUMNODE], float Output_C_Location[NUMNODE]);
...
void output();
// Superior Function
// void sys_reset(int Output_C_Distribution[NUMNODE], int Output_R_Distribution[NUMNODE], int Output_C_Location[NUMNODE]);
void sys_reset(float Output_C_Distribution[NUMNODE], float Output_R_Distribution[NUMNODE], float Output_C_Location[NUMNODE]);
// void execution(float Actions[NUMCAR], int time_id, int Output_C_Distribution[NUMNODE], int Output_R_Distribution[NUMNODE], int Output_C_Location[NUMNODE], float Output_Reward[1]);
void execution(float Actions[NUMCAR], int time_id, float Output_C_Distribution[NUMNODE], float Output_R_Distribution[NUMNODE], float Output_C_Location[NUMNODE], float Output_Reward[1]);
// void global_state_reset(int CS_Node[NUMCAR], int RL_ReqTime[NUMREQ], int RL_UpNode[NUMREQ], int Output_C_Distribution[NUMNODE], int Output_R_Distribution[NUMNODE]);
// void car_state_generate(int car_id, int Output_C_Location[NUMNODE]);
void car_state_generate(int car_id, float Output_C_Location[NUMNODE]);
