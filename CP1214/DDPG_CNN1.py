...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.conv1 = nn.Sequential(      
            nn.Conv2d(in_channels=state_dim, out_channels=32, kernel_size=(4,4), stride=1, padding=0),
            nn.ReLU(inplace=True),             #output_size: H*W = 19*37*32
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(2,2), stride=2, padding=1),
            nn.ReLU(inplace=True),              #output_size: H*W = 10*19*64
            # nn.MaxPool2d(kernel_size=3, stride=1)  #output_size: H*W = 3*6
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(2,3), stride=1, padding=0),
            nn.ReLU(inplace=True),              #output_size: H*W = 9*17*128
            # nn.MaxPool2d(kernel_size=3, stride=1)  #output_size: H*W = 3*6
        )
        self.fc1 = nn.Sequential(
            nn.Linear(19584, 128),        #input_size: 9*17*128=
            nn.ReLU()
        )
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fc_s2 = nn.Sequential(
            nn.Linear(128, action_dim),        #input_size: 9*17*128=
            nn.ReLU()
        )
        self.out = nn.Sequential(
...
if __name__ == '__main__':
    ...
    MAX_EP_STEPS = 100     #每个episode的步数，时间间隔初设为1分钟，请求发出持续到1440分钟，即1440step   
    MEMORY_CAPACITY = 1000   #记忆池容量，20000
    REPLACEMENT = [
    ...
    env = ll(r"/home/chwei/JeanLu/SharedTaxi/C_python/CP1214/libPolicy2_cnn.so")  
