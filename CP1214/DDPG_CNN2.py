...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.conv_s4 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(3,5), stride=1, padding=0),
            nn.ReLU(inplace=True),              #output_size: H*W = 9*17*128
            nn.MaxPool2d(kernel_size=3, stride=1)  #output_size: H*W = 6*22
        )
        self.fc_s1 = nn.Sequential(
            nn.Linear(24576, 128),        #input_size: 9*17*128=
            nn.ReLU()
        )
...
class DDPG(object):
    ...
    def learn(self):
        ...
        reward_batch = torch.from_numpy(np.reshape(np.array(reward_batch), (self.batch_size,1)).astype(np.float32))      #(32, 1)  
...
if __name__ == '__main__':
    ...
    MAX_EP_STEPS = 50    #每个episode的步数，时间间隔初设为1分钟，请求发出持续到1440分钟，即1440step   
    MEMORY_CAPACITY = 500   #记忆池容量，20000
    REPLACEMENT = [
    ...
    env = ll(r"/home/chwei/JeanLu/SharedTaxi/C_python/CP1214/libPolicy2_0212.so")  
    ...
    Car_Distribution_15 = np.empty((NUM_NODE), dtype = float) 
    Car_Distribution_30 = np.empty((NUM_NODE), dtype = float) 
    Req_Distribution = np.empty((NUM_NODE), dtype = float)   
    Req_Distribution_15 = np.empty((NUM_NODE), dtype = float)   
    Req_Distribution_30 = np.empty((NUM_NODE), dtype = float)   
    ...
    Car_Distribution_15 = p2c.ConvertListFlt(Car_Distribution_15) 
    Car_Distribution_30 = p2c.ConvertListFlt(Car_Distribution_30) 
    Req_Distribution = p2c.ConvertListFlt(Req_Distribution)
    Req_Distribution_15 = p2c.ConvertListFlt(Req_Distribution_15)
    Req_Distribution_30 = p2c.ConvertListFlt(Req_Distribution_30)
    ...
    for ep in range(MAX_EPISODES):
        ...
        env.sys_reset(Car_Distribution, Car_Distribution_15, Car_Distribution_30, Req_Distribution, Req_Distribution_15, Req_Distribution_30, Car_Location)
        #初始化episode的累计奖励
        ep_reward = 0       
        ...
        for t in range(MAX_EP_STEPS):     
            ...
            CarDistribution15_2D = np.zeros((H_Grid, W_Grid,), dtype = float)
            CarDistribution30_2D = np.zeros((H_Grid, W_Grid,), dtype = float)
            ReqDistribution_2D = np.zeros((H_Grid, W_Grid), dtype = float)
            ReqDistribution15_2D = np.zeros((H_Grid, W_Grid), dtype = float)
            ReqDistribution30_2D = np.zeros((H_Grid, W_Grid), dtype = float)
            ...
            for i in range(NUM_NODE):                
                ...
                CarDistribution15_2D[rows][i % 40] = np.array(Car_Distribution_15)[i]
                CarDistribution30_2D[rows][i % 40] = np.array(Car_Distribution_30)[i]
                ReqDistribution_2D[rows][i % 40] = np.array(Req_Distribution)[i]
                ReqDistribution15_2D[rows][i % 40] = np.array(Req_Distribution_15)[i]
                ReqDistribution30_2D[rows][i % 40] = np.array(Req_Distribution_30)[i]
            #获取全局共享状态：路网中车辆分布和请求分布
            s_global = np.stack((CarDistribution_2D, CarDistribution15_2D, CarDistribution30_2D, ReqDistribution_2D, ReqDistribution15_2D, ReqDistribution30_2D), axis = 0)   #(2, 22, 40)       
            # check
            # print("--------------------step", t, "--------------------------")
            # for i in range(NUM_NODE):
            ...
            env.execution(a_all_c, interval, Car_Distribution, Car_Distribution_15, Car_Distribution_30, Req_Distribution, Req_Distribution_15, Req_Distribution_30, Car_Location, Reward)
            ...
            CarDistribution15_2D_ = np.zeros((H_Grid, W_Grid), dtype = float)
            CarDistribution30_2D_ = np.zeros((H_Grid, W_Grid), dtype = float)
            ReqDistribution_2D_ = np.zeros((H_Grid, W_Grid), dtype = float)
            ReqDistribution15_2D_ = np.zeros((H_Grid, W_Grid), dtype = float)
            ReqDistribution30_2D_ = np.zeros((H_Grid, W_Grid), dtype = float)
            rows = -1
            for i in range(NUM_NODE):                
                if (i % 40 == 0):
                    rows += 1
                CarDistribution_2D_[rows][i % 40] = np.array(Car_Distribution)[i]
                CarDistribution15_2D_[rows][i % 40] = np.array(Car_Distribution_15)[i]
                CarDistribution30_2D_[rows][i % 40] = np.array(Car_Distribution_30)[i]
                ReqDistribution_2D_[rows][i % 40] = np.array(Req_Distribution)[i]
                ReqDistribution15_2D_[rows][i % 40] = np.array(Req_Distribution_15)[i]
                ReqDistribution30_2D_[rows][i % 40] = np.array(Req_Distribution_30)[i]
            #获取t+1全局共享状态：路网中车辆分布和请求分布
            s_global_ = np.stack((CarDistribution_2D_, CarDistribution15_2D_, CarDistribution30_2D_, ReqDistribution_2D_, ReqDistribution15_2D_, ReqDistribution30_2D_), axis = 0)   #(4,22,40)       
            # print("time id:", t)
            ...
            if ddpg.pointer > MEMORY_CAPACITY: 
                ...
                if t==0:
                    ...
                    print("action in t=10: ",a_all)  
                elif t==20:
                    print("action in t=20: ",a_all)  
                elif t==30:
                    print("action in t=30: ",a_all)  
                elif t==49:
                    print("action in t=49: ",a_all)  
                else:
                    pass
                # if t==0:
