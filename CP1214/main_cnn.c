...
int main()
{
    ...
    // begin execution
    for (t_interval = 0; t_interval < 96; t_interval++)
    {
...
*/
void execution(float Actions[NUMCHAIR + 1], int interval_id, float Car_Distribution[NUMNODE], float Car_Distribution_15[NUMNODE], float Car_Distribution_30[NUMNODE], float Req_Distribution[NUMNODE], float Req_Distribution_15[NUMNODE], float Req_Distribution_30[NUMNODE], float Car_Location[NUMNODE], float reward[1])
{
    ...
    int exec_reqids[1000] = {0}; // 存储指定时间间隔内的请求id    100!!!
    // 记录当前时间间隔内的请求id和请求数量
    for (j = 0; j < NUMREQ_RUN; j++)
    {
        if ((ReqList_ReqTime[j] >= interval_id * 15) && (ReqList_ReqTime[j] < (interval_id + 1) * 15)) // if (ReqList_ReqTime[j] == interval_id )
        {
    ...
    for (j = 0; j < req_num; j++)
    {
        ...
        // 对于每个请求，初始化TaskChain_Candi列表为-1
        for (i = 0; i < 1000; i++) // 50!!!
            TaskChain_Candi[i] = -1;
        ...
        {
            carid = near_cars[i];
            if (CarState_ChairNum[carid] > 3)
                printf("Awfully wrong!!!!");
            action_car = Actions[CarState_ChairNum[carid]];
    ...
    for (i = 0; i < NUMCAR; i++)
    {
        update_15((interval_id + 1) * 15, i);
        update_30((interval_id + 1) * 15, i);
    }
    for (car = 0; car < NUMCAR; car++)
    {
        Car_Distribution[CarState_NodeID[car]] += 1;
    ...
    j = 0;
    while (ReqList_ReqTime[j] < interval_id * 15)
    {
        if (ReqList_ReqTime[j] >= (interval_id * 15 - 15))
            Req_Distribution_15[ReqList_UpNode[j]] += 1;
        if (ReqList_ReqTime[j] >= (interval_id * 15 - 30))
