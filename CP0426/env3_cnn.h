...
void update(int req_id, int car_id)
{
    ...
    curtime = (float)CarState_Time[car_id];
    ...
    //     printf("req%d--car%d-- chair num:%d\n", req_id, car_id, chair_num);
    ...
    if (forwardtaskid >= 0)
    {
        ...
        while (forwardtime <= (float)ReqList_ReqTime[req_id])
        {                          /*需完成请求发出时刻之前所有可以完成的任务*/
            ...
            if (forwardtaskid % 2 == 0)
            {
                ...
                // if (isnan(car_incre_revd) == 1)
                // {
                //     printf("ReqList_UpNode[%d]:  %d\n", reqid_fromtask, ReqList_UpNode[reqid_fromtask]);
                //     printf("ReqList_OffNode[%d]:  %d\n", reqid_fromtask, ReqList_OffNode[reqid_fromtask]);
                // }
                ...
                // 燃油成本的计算，按照人数计费
                if (chair_num == 0)
                    car_incre_cost = car_incre_cost + FUELCOEF0 * MinCost[curnode][forwardnode];
                else if (chair_num == 1)
                    car_incre_cost = car_incre_cost + FUELCOEF1 * MinCost[curnode][forwardnode];
                else if (chair_num == 2)
                    car_incre_cost = car_incre_cost + FUELCOEF2 * MinCost[curnode][forwardnode];
                else if (chair_num == 3)
                    car_incre_cost = car_incre_cost + FUELCOEF3 * MinCost[curnode][forwardnode];
                // // 燃油成本的计算，不区分乘客数量
                // car_incre_cost = car_incre_cost + FUELCOEF2 * MinCost[curnode][forwardnode];
                ...
                if (chair_num > 3 || chair_num < 0)
                {
                    printf("上车--车辆人数异常！：%d \n", chair_num);
                    printf("问题出在   req: %d,  car: %d", req_id, car_id);
                    // exit(0);
                }
            }
            ...
            // if (forwardtaskid == TaskList_NextTaskID[forwardtaskid])
            // {
            //     printf("问题出在   req: %d,  car: %d", req_id, car_id);
            //     exit(0);
            // }
            // printf("before forwardtaskid: %d \n", forwardtaskid);

            forwardtaskid = TaskList_NextTaskID[forwardtaskid]; /*更新forwardtaskid*/

            // printf("after forwardtaskid: %d \n", forwardtaskid);
            ...
            else // 在下一请求发出时刻之前，该车所有的任务已经完成了
            {
                if (((float)ReqList_ReqTime[req_id] - curtime) < 0)
                    printf("time process is wrong!!!\n");
                ahead_or_not = 1; // 1代表该车提前完成任务，原地待命，故CostToNode应该=0
                break;            // 没有任务就退出While循环，重要
            }
        }
        /*此时要将车辆状态变量（位置、距上一节点时间、首个目标任务）→更新到→新请求发出时刻*/
        CarState_NodeID[car_id] = curnode;
        if (ahead_or_not == 1)
            CarState_CostToNode[car_id] = 0; // 避免CostToNode出现错误
        else
            CarState_CostToNode[car_id] = (float)ReqList_ReqTime[req_id] - (curtime - curcosttonode);
        CarState_FirstTask[car_id] = forwardtaskid;
        CarState_ChairNum[car_id] = chair_num;

        if (CarState_CostToNode[car_id] < 0)
            printf("CarState_CostToNode[%d] is wrong!!!\n", car_id);
    }
    ...
    // }
    // printf("car%d 's cost: %.3f\n", car_id, car_incre_cost);
    ...
    // if (isnan(car_incre_cost) == 1)
    // {
    //     printf("car_incre_cost: %f\n", car_incre_cost);
    //     printf("问题出在   car: %d,  req: %d\n", car_id, req_id);
    // }
    // if (isnan(car_incre_revd) == 1)
    // {
    //     printf("car_incre_revd: %f\n", car_incre_revd);
    //     printf("问题出在   car: %d,  req: %d\n", car_id, req_id);
    // }
}

/*match(): 状态时间已更新到新请求发出时刻，将新请求插入到车辆的任务清单中*/
/*match_policy1: 无差别地寻找最小损失的车辆*/
void match(int req_id, int car_id)
{
    ...
    int i, j, m;             // 循环变量
    int uploc;               // uptask插入的位置,一开始插入位置为1
    int offloc;              // offtask插入的位置,一开始插入位置为uptask后1个单位
    int tasknum;             // 存储未完成任务链的数量
    int visitloc;            // 对可能方案的任务链进行计算时，任务访问位置
    // 对于每个车辆，需要初始化TaskChain列表为-1
    for (i = 0; i < 50; i++) // 一定在改变改变数组长度后检查相关循环次数，防止溢出！！！
        TaskChain[i] = -1;
    // 对于每个车辆，初始化FinishTime_Temp为-1
    for (i = 0; i < NUMTASK_MAX; i++) // 在匹配方案计算过程中，完成时间的存储同时采用TaskList_FinishTime和Temp，更快方法？
    ...
    if (forwardtask < 0)
    {
        ...
        // 计算乘客损失时间
        waittime = MinCost[curnode][upnode];   // 车辆无任务执行时，默认其停留在curnode节点，直至接到下一个任务
        detourtime = 0;                        // 没有合乘过程，所以没有绕行时间
        dispatch_cost = waittime + detourtime; // 损失时间为两者之和
    ...
    else
    {
        ...
        if (nexttask < 0)
        {
            ...
            dispatch_cost = waittime + detourtime;
                ...
                { // 内层循环，后移offloc
                    ...
                    while (fwdtask_temp >= 0)
                    {
                        ...
                        if ((fwdtask_temp % 2) == 0)
                        {
                            ...
                            // 计算该上车任务的等待时间
                            waittime = curtime - (float)ReqList_ReqTime[reqid_fromtask];
                            // 存储任务链的累加等待时间
                            waittime_all = waittime_all + waittime;
                            wt = wt + 1; // 累加次数+1

                            // 判断单个上车任务的等待时间是否超限
                            if (waittime > WAITTHRESHOLD || waittime < 0) // 注意：此处作了简单处理，相当于进入第二天的有客车辆不会接单？？？
                            {
                                feasible_switch = 0;
                        ...
                        else
                        {
                            ...
                            if (detourtime > DETOURTHRESHOLD || detourtime < -1)
                            {
                                feasible_switch = 0;
                                break;
                            }
                    ...
                    dispatch_cost = (waittime_all / wt) + (detourtime_all / dt); // 由于按照每个请求的平均等待/平均绕行，可能出现有的请求等待时间很长

                    // 判断是否需要将temp写入candi
                    if ((dispatch_cost <= Min_Dispatch_Cost) && (feasible_switch == 1))
                    {
                        if (dispatch_cost < 0)
                            printf("wrong!!!:  car%d  dispatch_cost: %f   req: %d\n", car_id, dispatch_cost, req_id);
                        // 更新最小损失时间
                        Min_Dispatch_Cost = dispatch_cost;
                        // 更新最优车辆ID
                        BestCar = car_id;
...
// 插入排序法：对所有附近车辆按照距离升序排序
void insert_sort_dis(int cars_after_sort[], int reqid)
{
    int i, j, mm;                     // 循环变量
    int num = 0;                      // 搜索获得的附近车辆数
    int near_cars[NUMCAR];            // 符合搜索范围的全部车辆id
    float distance_to_upnode[NUMCAR]; // 存储到目标站点距离
    // 初始化near_cars[]为-1
    for (i = 0; i < NUMCAR; i++)
        near_cars[i] = -1;
    // 搜索附近车辆
    for (i = 0; i < NUMCAR; i++)
    {
        if (MinCost[CarState_NodeID[i]][ReqList_UpNode[reqid]] <= SEARCH_DIS)
        {
            near_cars[num] = i;                                                           // 获取符合距离条件内的车辆ID
            distance_to_upnode[num] = MinCost[CarState_NodeID[i]][ReqList_UpNode[reqid]]; // 存储到目标站点距离
            num++;                                                                        // 附近车数+1
        }
    }
    // 对【满足搜索范围的全部车辆】进行距离升序排序
    for (i = 1; i < num; i++)
    {
        // 用每个车辆id对应的距离作为排序依据
        if (distance_to_upnode[i] < distance_to_upnode[i - 1])
        { // 若第 i 个元素大于 i-1 元素则直接插入；反之，需要找到适当的插入位置后在插入。
            j = i - 1;
            int idx = near_cars[i];             // 保存哨兵id
            float disx = distance_to_upnode[i]; // 保存哨兵distance
            ...
            { // 采用顺序查找方式找到插入的位置，在查找的同时，将数组中的元素进行后移操作，给插入元素腾出空间
                ...
                near_cars[j + 1] = near_cars[j];
                j--;
            }
            distance_to_upnode[j + 1] = disx;
            near_cars[j + 1] = idx; // 插入到正确位置
        }

        // printf("第%d次排序结果：", i);
        // for (mm = 0; mm < MAX_MATCH_NUM; mm++)
        // {
        //     printf("%d ", near_cars[mm]); // 打印每次排序后的结果
        //     if (near_cars[mm + 1] == -1)
        //         break;
        // }
        // printf("\n");
    }
    // (如果超出最大附近车辆数，则按照顺序进行截取
    if (num >= MAX_MATCH_NUM)
    {
        for (i = 0; i < MAX_MATCH_NUM; i++)
            cars_after_sort[i] = near_cars[i];
    }
    else
    {
        for (i = 0; i < num; i++)
    ...
    // printf("cars_sort_dis[]:\n");
    // for (mm = 0; mm < MAX_MATCH_NUM; mm++)
    // {
    //     printf("%d ", cars_after_sort[mm]);
    //     if (cars_after_sort[mm + 1] == -1)
    //         break;
    // }
    // printf("\n");
}

// 插入排序法：对所有附近车辆按照空座数升序排序
void insert_sort_psg(int cars_after_sort[], int reqid)
{
    int i, j, mm; // 循环变量
    // cars_after_sort数组已经完成初始化（距离升序）
    // 开始按乘客数排序
    for (i = 1; i < MAX_MATCH_NUM; i++)
    {
        if (cars_after_sort[i] < 0) // 如果读取到-1（结尾）即结束排序
            break;
        // 用每个车辆id对应的距离作为排序依据
        if (CarState_ChairNum[cars_after_sort[i]] > CarState_ChairNum[cars_after_sort[i - 1]])
        { // 若第 i 个元素大于 i-1 元素则直接插入；反之，需要找到适当的插入位置后在插入。
            j = i - 1;
            int idx = cars_after_sort[i];                       // 保存哨兵id
            float disx = CarState_ChairNum[cars_after_sort[i]]; // 保存哨兵distance
            while (j > -1 && disx > CarState_ChairNum[cars_after_sort[j]])
            { // 采用顺序查找方式找到插入的位置，在查找的同时，将数组中的元素进行后移操作，给插入元素腾出空间
        ...
        // printf("第%d次排序结果：", i);
        // for (mm = 0; mm < MAX_MATCH_NUM; mm++)
        //     printf("%d ", cars_after_sort[mm]); // 打印每次排序后的结果
        // printf("\n");
        // for (mm = 0; mm < MAX_MATCH_NUM; mm++)
        //     printf("%d ", CarState_ChairNum[cars_after_sort[mm]]); // 打印每次排序后的结果
        // printf("\n");
    }
}

// 搜索附近车辆，并将车辆分为空车和非空车两个集合
void vacant_and_occupied(int cars_vacant[], int *num_vacant, int cars_occupied[], int *num_occupied, int reqid)
{
    int i, j, m;
    int num = 0;           // 搜索获得的附近车辆数
    int near_cars[NUMCAR]; // 符合搜索范围的全部车辆id
    ...
    // 忘记考虑num> MAX_MATCH_NUM的情况,导致指针溢出!!!    ------bug查找用时2天
    if (num > MAX_MATCH_NUM)
        num = MAX_MATCH_NUM;
    // 按照是否载客进行分类
    i = 0; // 计数前清零
    ...
    for (m = 0; m < num; m++)
    { // 空车：当前无乘客且无任务
        if (CarState_ChairNum[near_cars[m]] == 0 && CarState_FirstTask[near_cars[m]] == -1)
        {
            cars_vacant[i] = near_cars[m];
            i++;
        }
        else
        {
            cars_occupied[j] = near_cars[m];
            j++;
        }
    }
    *num_vacant = i; // 存储两类车辆的数量
    *num_occupied = j;
}
...
void output()
{
    int i = 0, j = 0;
    // 创建文件指针
    // FILE *fp_RouteList = fopen("/home/chwei/JeanLu/DataOutput/0426/Car_RouteList_400p1.csv", "w+");
    FILE *fp_Chair = fopen("/home/chwei/JeanLu/DataOutput/0426/TaskList_Chair_400p1.csv", "w+");
    FILE *fp_FinishTime = fopen("/home/chwei/JeanLu/DataOutput/0426/TaskList_FinishTime_400p1.csv", "w+");
    FILE *fp_Cost_Reward = fopen("/home/chwei/JeanLu/DataOutput/0426/ReqList_Cost_Reward_400p1.csv", "w+");
    FILE *fp_WaitTimeOrDetourTime = fopen("/home/chwei/JeanLu/DataOutput/0426/TaskList_WaitTimeOrDetourTime_400p1.csv", "w+");
    FILE *fp_NextTask = fopen("/home/chwei/JeanLu/DataOutput/0426/TaskList_NextTaskID_400p1.csv", "w+");
    // 出错处理,避免传入空指针 (fp_RouteList == NULL) ||
    if ((fp_Chair == NULL) || (fp_FinishTime == NULL) || (fp_Cost_Reward == NULL) || (fp_WaitTimeOrDetourTime == NULL) || (fp_NextTask == NULL))
    {
        printf("打开错误...");
        return;
    }
