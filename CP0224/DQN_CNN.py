...
def save_parameters():
    ...
    torch.save(obj=qnet_paras, f="CP0224/qnet_paras_save_dqn.pth")

# 重载模型参数
def reload_parameters():
    #state_dict() 只保存网络参数，不保存网络结构
    reload_states = torch.load("CP0224/qnet_paras_save_dqn.pth")#重载的reload_states 为一个字典
...
if __name__ == '__main__':
    ...
    memory_capacity =10000   #记忆池容量，一般设为1e6
    ...
    for ep in range(MAX_EPISODES):
        ...
        for t in range(MAX_EP_STEPS):     
            ...
            #获取动作值，并转换数据类型为float
            a = dqn.take_action(s_global, epsilo)        #返回的a为numpy.int

            # max_q_value = dqn.max_q_value(s_global) * 0.005 + max_q_value *0.995 #平滑处理
            max_q_value = dqn.max_q_value(s_global)
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('DQN on env')
    plt.savefig('CP0224/return_DQN{}.png'.format(save_date))
    plt.show()
    #可视化每个状态的max_q_value
    ...
    plt.ylabel('Q values')
    plt.title('DQN on env')
    plt.savefig('CP0224/Qvalue_DQN{}.png'.format(save_date))
    plt.show()
