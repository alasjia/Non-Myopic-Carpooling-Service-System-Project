...
class VAnet(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fc2A = nn.Sequential(
            nn.Linear(128, action_dim)                   #此处不应当有relu函数吧？？？
        )
    ...
    def forward(self, x):
        ...
        V = self.fc1V(x)   #状态价值头
        V = self.fc2V(V)
        Q = V + A - A.mean(1).view(-1, 1)  #mean(1)对各行求均值
        return Q
...
class DuelingDQN(object):
    ...
    def take_action(self, state, epsilon):
        if np.random.random() < epsilon:
            ...
            action = self.q_net(state).argmax().item()  #选择最大值对应的序号【0、1、2】;
        return action

    def max_q_value(self, state):
        state = torch.from_numpy(state).float()            
        state = torch.unsqueeze(state, dim=0).to(self.device) #增加一个维度用于输入CNN
        # state = torch.tensor([state], dtype = torch.float32).to(self.device)   #这句作用与上两行作用相同，但有warning：使用list创建tensor速度慢
    ...
    def learn(self, transition_dict):
        ...
        max_next_q_values = self.q_net_target(state_batch_).max(1)[0].view(-1, 1)     #view(-1,1)将(batch,)变为(batch,1)   max(1)对每行进行操作后有values和indices两行值，所以[0]
        # print(max_next_q_values)
        q_targets = reward_batch + self.gamma * max_next_q_values     #TD误差目标
        agent_loss = torch.mean(self.mse_loss(q_values, q_targets))
...
def save_parameters():
    ...
    torch.save(obj=qnet_paras, f="CP0224/qnet_paras_save0314.pth")

# 重载模型参数
def reload_parameters():
    #state_dict() 只保存网络参数，不保存网络结构
    reload_states = torch.load("CP0224/qnet_paras_save0314.pth")#重载的reload_states 为一个字典
...
if __name__ == '__main__':
    ...
    memory_capacity =10000   #记忆池容量，一般设(为1e6
    minimal_size = 8000       #累积minimal_size条经验后开始训练
    lr = 0.001
    ...
    a_dim = 3       
    # 设置agent
    agent = DuelingDQN(state_dim=s_dim,
                action_dim=a_dim,
                gamma = gamma,
                learning_rate = lr,
    ...
    for ep in range(MAX_EPISODES):
        ...
        for t in range(MAX_EP_STEPS):     
            ...
            max_q_value = agent.max_q_value(s_global)
    ...
    #可视化每个episode的Reward
    save_date = 'test'   #今天是几月几号呢:)
    episodes_list = list(range(len(return_list)))
    plt.plot(episodes_list, return_list)
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('DuelingDQN on env')
    plt.savefig('CP0224/return_DuelingDQN{}.png'.format(save_date))
    plt.show()
    #可视化每个状态的max_q_value
    ...
    plt.ylabel('Q values')
    plt.title('DuelingDQN on env')
    plt.savefig('CP0224/Qvalue_DuelingDQN{}.png'.format(save_date))
    plt.show()
