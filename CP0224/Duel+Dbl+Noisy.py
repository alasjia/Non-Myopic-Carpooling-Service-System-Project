...
#创建噪声层
'''
Q1：为什么做决策时没有将DQN的噪声消除？？？
Q2：对于DuelingNet用在哪合适？
Q3：噪声层内部没有细看
'''
...
class VAnet(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fc2A = NoisyFactorizedLinear(128, action_dim)
        self.fc1V = nn.Sequential(
            nn.Linear(10*28*32, 128),        
            nn.ReLU()
        )
        self.fc2V =  NoisyFactorizedLinear(128, 1)
...
def save_parameters():
    ...
    torch.save(obj=qnet_paras, f="CP0224/qnet_paras_duel+dbl+noisy.pth")

# 重载模型参数
def reload_parameters():
    #state_dict() 只保存网络参数，不保存网络结构
    reload_states = torch.load("CP0224/qnet_paras_duel+dbl+noisy.pth")#重载的reload_states 为一个字典
...
if __name__ == '__main__':
    ...
    H_Grid = 22              #地理网格高度
    W_Grid = 40             #地理网格宽度
    MAX_EPISODES = 50#100           #运行次数（天数）
    MAX_EP_STEPS = 500#1000    #每个episode的步数，时间间隔初设为1分钟，请求发出持续到1440分钟，即1440step   
    memory_capacity = 10000#50000   #记忆池容量，一般设为1e5-6，10000有点小
    minimal_size = 8000#30000       #累积minimal_size条经验后开始训练，1000设置的太小
    lr = 0.001
    gamma = 0.99              #折扣因子，越大->未来奖励影响  合理的小->敢于冒险
    epsilo = 1        #初始随机率，1等于完全随机选择
    decay_rate = 0.999#0.9999      # 决定衰减周期
    target_update  = 100#200    #延迟更新频率
    batch_size = 32
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('DQN with Double+Dueling+NoisyNet on env')
    plt.savefig('CP0224/return_Duel+Dbl+Noisy_{}.png'.format(save_date))
    plt.show()
    #可视化每个状态的max_q_value
    ...
    plt.ylabel('Q values')
    plt.title('DQN with Double+Dueling+NoisyNet on env')
    plt.savefig('CP0224/Qvalue_Duel+Dbl+Noisy_{}.png'.format(save_date))
    plt.show()
