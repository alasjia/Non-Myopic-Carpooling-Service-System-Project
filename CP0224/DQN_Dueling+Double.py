...
def save_parameters():
    ...
    torch.save(obj=qnet_paras, f="CP0224/qnet_paras_duel+double.pth")

# 重载模型参数
def reload_parameters():
    #state_dict() 只保存网络参数，不保存网络结构
    reload_states = torch.load("CP0224/qnet_paras_duel+double.pth")#重载的reload_states 为一个字典
...
if __name__ == '__main__':
    ...
    memory_capacity =100000   #记忆池容量，一般设为1e5-6，10000有点小
    minimal_size = 70000       #累积minimal_size条经验后开始训练，1000设置的太小
    lr = 0.001
    gamma = 0.99              #折扣因子，越大->未来奖励影响  合理的小->敢于冒险
    epsilo = 1        #初始随机率，1等于完全随机选择
    decay_rate = 0.9999      # 决定衰减周期
    target_update  = 500    #延迟更新频率
    batch_size = 32
    ...
    env = ll(r"/home/chwei/JeanLu/SharedTaxi/C_python/CP0224/lib1000.so")  
    ...
    #可视化每个episode的Reward
    save_date = '0404'   #今天是几月几号呢:)
    episodes_list = list(range(len(return_list)))
    plt.plot(episodes_list, return_list)
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('DQN with Double+Dueling on env')
    plt.savefig('CP0224/return_DQN_Dueling+Double_{}.png'.format(save_date))
    plt.show()
    #可视化每个状态的max_q_value
    ...
    plt.ylabel('Q values')
    plt.title('DQN with Double+Dueling on env')
    plt.savefig('CP0224/Qvalue_DQN_Dueling+Double_{}.png'.format(save_date))
    plt.show()
